...
# pipelineDirectory: path to the circularRNApipeline directory.
...
# indexDirectory: optional output directory for KNIFE index files, default: ${pipelineDirectory}/index
...
then
  ...
  INDEX_DIR=${PIPELINE_DIR}/index
fi

mkdir -p ${INDEX_DIR}

python "${SCRIPT_DIR}/makeJunctionsAndWriteFasta.py" -w ${WINDOW} -e ${OUT_DIR}/exons -r ${OUT_DIR}/records -f ${OUT_DIR}/fastas -n1 ${GENE_NAME_1} -n2 ${GENE_NAME_2} -v
...
python "${SCRIPT_DIR}/limitFasta.py" -s ${OUT_DIR}/${FILE_ID}.fa -o ${OUT_DIR}/fastas/ -t reg -p _junctions_reg
python "${SCRIPT_DIR}/limitFasta.py" -s ${OUT_DIR}/${FILE_ID}.fa -o ${OUT_DIR}/fastas/ -t dup -p _junctions_dup
python "${SCRIPT_DIR}/limitFasta.py" -s ${OUT_DIR}/${FILE_ID}.fa -o ${OUT_DIR}/fastas/ -t rev -p _junctions_rev

# combine rev and dup junctions into scrambled fasta file and put in the index directory
echo "cat ${OUT_DIR}/fastas/${FILE_ID}_junctions_rev.fa ${OUT_DIR}/fastas/${FILE_ID}_junctions_dup.fa > ${INDEX_DIR}/${FILE_ID}_junctions_scrambled.fa"
cat ${OUT_DIR}/fastas/${FILE_ID}_junctions_rev.fa ${OUT_DIR}/fastas/${FILE_ID}_junctions_dup.fa > ${INDEX_DIR}/${FILE_ID}_junctions_scrambled.fa

# put the regular junction fasta file into the index directory
mv ${OUT_DIR}/fastas/${FILE_ID}_junctions_reg.fa ${INDEX_DIR}
...
echo "bowtie2-build ${INDEX_DIR}/${FILE_ID}_junctions_scrambled.fa ${INDEX_DIR}/${FILE_ID}_junctions_scrambled"
bowtie2-build ${INDEX_DIR}/${FILE_ID}_junctions_scrambled.fa ${INDEX_DIR}/${FILE_ID}_junctions_scrambled

# create linear junction bowtie2 index in index directory 
echo "bowtie2-build ${INDEX_DIR}/${FILE_ID}_junctions_reg.fa ${INDEX_DIR}/${FILE_ID}_junctions_reg"
bowtie2-build ${INDEX_DIR}/${FILE_ID}_junctions_reg.fa ${INDEX_DIR}/${FILE_ID}_junctions_reg
